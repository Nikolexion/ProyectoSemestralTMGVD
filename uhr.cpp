...
int main(int argc, char *argv[])
{
    ...
    std::vector<double> q;
    double mean_time, time_stdev, dev, mean_time2, time_stdev2, dev2;
    ...
    multi_countsketch mcs(3, new int[3]{21, 31, 51}, 1 << 26, 5);
    
    // File to write time data
    std::ofstream time_data;
    time_data.open(argv[1]);
    time_data << "n,t_mean,t_stdev,t_mean2,t_stdev2" << std::endl;
    ...
    for (n = lower; n <= upper; n += step) {
        ...
        for (i = 0; i < runs; i++) {
            ...
            for (const auto& path : archivos) {
                ...
                double score = mcs.calculate_score(secuencia, {1.0, 1.5, 2.0});
            }
        ...
        time_stdev2 /= runs - 1; // Subtract 1 to get unbiased estimator
        time_stdev2 = std::sqrt(time_stdev2);

        time_data << n << "," << mean_time << "," << time_stdev << ",";
        time_data << mean_time2 << "," << time_stdev2 << std::endl;
    }
